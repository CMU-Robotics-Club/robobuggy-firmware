...
void loop()
{
  ...
  /* ================ */
    ...
    if (std::optional<host_comms::SCRawGPS> gps_coord = host_comms::sc_gps())
    {
      if (!kalman_init && gps_coord->accuracy < 50.0)
      {
        ...
        filter.curr_state_est(0, 0) = gps_coord->eastern;
        filter.curr_state_est(1, 0) = gps_coord->northern;
      ...
      if (kalman_init)
      {
        ...
        filter.update_gps(measurement_vector_t{gps_coord->eastern, gps_coord->northern});
      }
    }
  ...
  // Orange by default
  status_led::Rgb status_color = status_led::orange;
  ...
  if (rc::use_autonomous_steering())
  {
    ...
    else
    {
      ...
      case AlarmStatus::Warning:
        // Blink blue/orange
        status_color = ((millis() % 1000) < 500) ? status_led::blue : status_led::orange;
        break;
      case AlarmStatus::Error:
        // Blink blue/red
        status_color = ((millis() % 1000) < 500) ? status_led::blue : status_led::red;
        break;
      default:
        break;
      }
  ...
  else
  {
    ...
    if (host_comms::message_age() <= 1000)
    {
      ...
      case AlarmStatus::Ok:
        // Green
        status_color = status_led::green;
        break;
      case AlarmStatus::Warning:
        // Blink green/orange
        status_color = ((millis() % 1000) < 500) ? status_led::green : status_led::orange;
        break;
      case AlarmStatus::Error:
        // Blink green/red
        status_color = ((millis() % 1000) < 500) ? status_led::green : status_led::red;
        break;
      default:
        break;
      }
  ...
  if ((steering::alarm_triggered() == steering::Status::alarm) || !rc::connected())
  {
    ...
    status_color = ((millis() % 300) < 150) ? status_led::red : status_led::black;
  }
  ...
  if (radio_available())
  {
    ...
    if (std::optional<uint8_t> length = radio_receive(buf))
    {
      ...
      if (p->tag == GPS_X_Y)
      {
        ...
        nand_pkt.rx_rssi = radio_last_rssi();
